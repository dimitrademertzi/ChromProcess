...
def peak_indices_from_file(peak_file, chromatogram):
    ...
    for p_rt in peak_retention_times:
        ...
        idx = (
            ...
            - 2
            + chromatogram.signal[
                nearest_value_idx - 2 : nearest_value_idx + 2
            ].argmax()
        )
...
def peak_boundaries_from_file(chromatogram, peak_file):
    ...
    peak_starts = np.empty(
        0, dtype="int64"
    )  # the functions written to find the start and end of the peak relies on indexes, so we need to convert the retention times to their corresponding indexes.
